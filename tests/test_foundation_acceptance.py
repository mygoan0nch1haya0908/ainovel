...
def test_readiness_revision_matches_the_migration_head() -> None:
    ...
    assert getattr(database, "ALEMBIC_HEAD_REVISION", None) == "0003_draft_repair"
    assert ScriptDirectory.from_config(config).get_current_head() == "0003_draft_repair"
...
def test_migration_round_trip_creates_current_schema(
    ...
    try:
        ...
        assert _revision_number(database_url) == "0003_draft_repair"
